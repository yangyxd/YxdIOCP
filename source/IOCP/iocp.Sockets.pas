...
begin
  ...
  if (FErrorCode = 10053) or (FErrorCode = 10054) then begin
    Disconnect; // 连接断开
    if RaiseErr then
...
function TIocpCustomBlockTcpSocket.ReadStream(OutStream: TStream;
  ...
  procedure CheckSocketState(const V: Integer);
  begin
    if V = -1 then begin
      FErrorCode := GetLastError;
      if (FErrorCode = 10053) or (FErrorCode = 10054) then
        Disconnect
      else
        SysUtils.RaiseLastOSError(FErrorCode);
    end;
